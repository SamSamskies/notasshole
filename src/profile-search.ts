...
export type ProfileSuggestion = ProfileInfo & {
  pubkey: string
  npub: string
  nip05?: string
}
...
function loadProfiles(term: string, limit: number): Promise<ProfileSuggestion[]> {
...
export function storeProfileSearchCacheForTest(
  query: string,
  limit: number,
  results: ProfileSuggestion[],
  fetchedAt: number,
): void {
  cache.set(cacheKey(query, limit), { results, fetchedAt })
}

/** @internal Test helper for cache expiry behavior. */
export function readProfileSearchCacheForTest(
  query: string,
  limit: number,
): ProfileSuggestion[] | null {
  return readCache(cacheKey(query, limit))
}
